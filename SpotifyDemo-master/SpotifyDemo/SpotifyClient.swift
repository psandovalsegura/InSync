...
class SpotifyClient {
    ...
    class func addToSavedTracks(parseTrack: PFObject, completionHandler: Void -> Void) {
        
        var trackIds: [String] = [String]()
        let currentTrackId = parseTrack["id"] as! String
        trackIds.append( currentTrackId )
        let trackPayload = ["ids": trackIds]
        
        Alamofire.request(.PUT, "https://api.spotify.com/v1/me/tracks", parameters: trackPayload, encoding: .JSON ,headers: ["Authorization": "Bearer \(SpotifyClient.CURRENT_SESSION.accessToken)", "Content-Type": "application/json"]).responseJSON { (response) in
